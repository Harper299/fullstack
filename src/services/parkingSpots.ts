...
export async function addParkingSpot(
  ...
  try {
    ...
    const spotRef = await addDoc(collection(db, SPOTS_COLLECTION), {
      reporterId: user.id,
...
// Upload spot photo
async function uploadSpotPhoto(file: File): Promise<string> {
  try {
    const filename = `spots/${Date.now()}_${file.name}`;
    const storageRef = ref(storage, filename);
    await uploadBytes(storageRef, file);
    return await getDownloadURL(storageRef);
  } catch (error) {
    console.error('Error uploading spot photo:', error);
    throw error;
  }
}
